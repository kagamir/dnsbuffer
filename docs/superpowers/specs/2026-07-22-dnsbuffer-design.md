...
7. 乐观缓存：缓存解析过的地址，配置限制条数，LRU 规则；命中直接返回，随后调用上游更新并重新入队，删除旧缓存。（原始需求提及 SQLite/FIFO，经讨论为响应速度改用纯内存链式哈希表，后经用户确认淘汰策略调整为 LRU，见第 3、7.2 节。）
8. 支持 EDNS 客户端子网（ECS）。
...
- **命中即返回**（不管 TTL 是否过期），保证低延迟；读取用 `to_back` 把条目刷新为最近使用（**LRU**）。
- **过期命中**额外触发后台刷新：上游拿到新结果后 `remove(key)` 再 `insert` 到队尾（O(1)）。
- **LRU 限条数**：插入后若总数超过配置上限，`pop_front` 淘汰最久未用条目（O(1)）。
...
- **单元测试**：配置解析、加权选择分布、缓存 LRU 淘汰与乐观刷新、filter/hosts 匹配、DNS 编解码往返、ECS 注入。
