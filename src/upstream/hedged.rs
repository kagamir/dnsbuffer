...
/// 对冲式快速重试：一次尝试超过 interval 未返回，就并行发起新尝试而不取消在途的
/// （每次尝试重新走内层的加权选择，大概率换成员）；任一成功即胜出。
/// 全部在飞尝试都失败时立即报错（失败不是慢，交给上层 fallback）；
/// 超过 max_wait 预算仍无结果则放弃。胜出后落败的在途尝试自然跑完（不打断连接）。
...
impl HedgedResolver {
    ...
    fn spawn_attempt(
        &self,
        query: &Message,
        tx: &tokio::sync::mpsc::UnboundedSender<Result<Message>>,
    ) {
        let inner = self.inner.clone();
        let query = query.clone();
        let tx = tx.clone();
        tokio::spawn(async move {
            // 胜者已定后通道关闭，send 失败可忽略：尝试本身照常跑完
            let _ = tx.send(inner.resolve(&query).await);
        });
    }
}
...
impl Resolver for HedgedResolver {
    async fn resolve(&self, query: &Message) -> Result<Message> {
        ...
        self.spawn_attempt(query, &tx);
        loop {
            tokio::select! {
                biased;
                res = rx.recv() => {
                    in_flight -= 1;
                    match res.expect("tx kept alive in this scope") {
                        Ok(resp) => return Ok(resp),
                        Err(e) if in_flight == 0 => return Err(e),
                        Err(e) => tracing::debug!("hedged attempt failed, others in flight: {e:#}"),
                    }
                }
